...
subtitle: "Analysis of known-winning starter words"
...
```{r}
# Initialize our word list: 2315 words (Wordle)
...
For this starter word list we use the results of running the `Autoplay.Rmd` notebook against the entire Wordle "Magic Words List," keeping only those words that when used as WordleR starters will result in a "win" in six steps or less. 

```{r}

trial_words <- c("inert","opera","resin","arson","cedar","coral",
                 "metal","recap","decal","ripen","octal","pedal",
                 "rayon","reign","scent","fecal","omega","medal",
                 "spent","guest","rearm","lyric","begat","chest",
                 "lemon","demon","swept","ingot","ghost",
                 "human","awake","usurp")
...
p1 <- ggplot(iteration_results_starter_success, 
  ...
  labs(
    ...
    subtitle = "Testing known-winning Wordle starter words",
    caption = "Wordle challenge words found in six iterations or less\nBased on the full 2315-element Wordle 'Magic Words' list\nVisit WordleR at: http://bit.ly/WordleR"
    ) +
  xlab("WordleR Starter Word") + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
  ylab("Wordle Challenge Words Found") +
  geom_hline(aes(yintercept = 2315),linetype = "dotdash") +
  geom_text(label="Wordle Magic Word List (truncated): 1829 words", x="equal", y=1829 + 1.5, size=3 )
...
According to this plot, the starter word **RECAP** _should never lose_ when used with the WordleR algorithm! 
